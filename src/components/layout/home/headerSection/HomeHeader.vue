...
import { ImageType, LocationType, VideoType } from '@jellyfin/client-axios';
...
// TODO(migration): Referencing the Swiper makes everything shit itself. Figure out why.

/* const onSlideChange = () => {
  currentIndex.value = swiper.value?.$data.swiperElRef;
  if (swiper.value?.isBeginning || swiper.value?.isEnd) {
    swiper.value?.$data.swiperElRef.updateSlides();
  }
  // TODO(migration): Migrate when backdrop is done
  /*const hash =
        this.getBlurhash(
...
const onAnimationEnd = () => {
  swiper.value?.$data.swiperElRef.slideNext();
};

const onProgressClicked = (index: number) => {
  swiper.value?.$data.swiperElRef.slideToLoop(index);
};
 */
const items = ref([
  {
...
import { BaseItemDto, LocationType, VideoType } from '@jellyfin/client-axios';
